...
class DegreeTab extends React.Component {
   render() {
    ...
    let viewdetails = courses && courses.length > 0 ? <div><Link target="_blank" href={"/course?course_level=2"} className="viewallbtn">View All Related Courses List</Link></div> : null;
    if(courses && courses.length > 0){courses.push(viewdetails)}
     return (
       <>
         <div className="tab-pane fade show active" id="tab2">
             {courses && courses.length !==1 ? courses  : 'No Records Found'}
         </div>
     </>
     );
   }
