...
class OthersTab extends React.Component {
  render() {
   ...
   const otherCourse = this.props.course;
   const courses = otherCourse.map( (item,i) =>
            ...
            <Link target="_blank" href={"/course/"+item.sef_url}>
               <div className="card-body card-one">
                  ...
                  <div className="detail detailCareer pt-2">
                     ...
                     <span><Image width={20} height={20} src="/images/study-mode.png" alt="img" className="img-fluid" /> {item.course_level_id && Util.singleLabel(model, 'course_level', item.course_level_id)} </span> 
   ...
   let viewdetails = courses && courses.length > 0 ? <div><Link target="_blank" href="/course?course_level=6" className="viewallbtn">View More</Link></div> : null;
